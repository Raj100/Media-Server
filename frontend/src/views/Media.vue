      ...
      <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">Browse your collection of movies, music, and videos</p>
          </div>
          <router-link
            to="/"
    ...
    <div class="container mx-auto px-4 py-6">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        >
          ...
          <div class="p-4">
            <h3 class="font-semibold text-gray-900 dark:text-white mb-1 truncate">{{ item.title }}</h3>
            <p v-if="item.type === 'music' && 'artist' in item" class="text-sm text-gray-600 dark:text-gray-400 mb-2">
              {{ (item as Music).artist }}
            </p>
            <p v-if="item.type === 'movie' && 'director' in item" class="text-sm text-gray-600 dark:text-gray-400 mb-2">
      ...
      <div v-if="filteredMedia.length === 0 && !mediaStore.isLoading" class="text-center py-12">
        <svg class="w-16 h-16 text-gray-400 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2h4a1 1 0 110 2h-1v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6H3a1 1 0 110-2h4z" />
        </svg>
        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No media found</h3>
...
const filteredMedia = computed((): MediaItem[] => {
  ...
  if (selectedType.value === "movie") {
    ...
    items = [...mediaStore.filteredMovies, ...mediaStore.filteredMusic, ...mediaStore.filteredVideos]
  }
